...
async function findAnyContactByEmail(
  email
) {
  const cleanEmail =
...
function getRowWarnings(row) {
  ...
  // ========================================
  // ALLEEN FOTOGRAAF VERPLICHT
  // ALS DE BOEKING ER ÉÉN NODIG HEEFT
  // ========================================

  if (
    row.requires_photographer !==
      false &&
    !cleanString(
...
export async function previewSimplyBookImport(
  ...
  for (
    ...
    const requiresPhotographer =
      row.requires_photographer !==
      false;

    const existingPhotographer =
      requiresPhotographer
        ? findPhotographer(
            fotografen,
            row.photographer_name
          )
...
async function createImportedBooking({
  ...
  // Alleen invullen als de boeking
  // daadwerkelijk een fotograaf heeft.
  if (photographer?.id) {
    properties
  ...
  try {
    ...
    // Fotograaf alleen koppelen
    // wanneer er daadwerkelijk één is.
    if (photographer?.id) {
      await associateBookingWithContactType(
        booking.id,
        photographer.id,
